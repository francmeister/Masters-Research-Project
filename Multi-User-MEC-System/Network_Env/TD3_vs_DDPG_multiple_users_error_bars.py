...
rewards_throughput_energy_DDPG_11_users = np.load('timestep_rewards_energy_throughput_DDPG_11_users.npy')


overall_users_reward_TD3_11_users = rewards_throughput_energy_TD3_11_users[:,3] - 10**8*rewards_throughput_energy_TD3_11_users[:,2] - 10**5*rewards_throughput_energy_TD3_11_users[:,4]
overall_users_reward_DDPG_11_users = rewards_throughput_energy_DDPG_11_users[:,3] - 10**8*rewards_throughput_energy_DDPG_11_users[:,2] - 10**5*rewards_throughput_energy_DDPG_11_users[:,4]
...
axs[0].plot(timesteps[window_size-1:], overall_users_reward_TD3_11_users_smooth, color="green", label="TD3")
axs[0].plot(timesteps[window_size-1:], overall_users_reward_DDPG_11_users_smooth, color="red", label='DDPG')

axs[0].set_xlabel("Episodes",fontsize=xlabel_fontsize)
axs[0].set_ylabel("Reward",fontsize=ylabel_fontsize)
axs[0].legend(loc="lower right",fontsize=legend_fontsize)
axs[0].grid()
axs[0].set_title("System Reward (TD3 vs DDPG)",fontsize=title_fontsize, fontweight='bold')
axs[0].tick_params(axis='x', labelsize=x_and_y_tick_fontsize)
axs[0].tick_params(axis='y', labelsize=x_and_y_tick_fontsize)
...
axs[1].errorbar(user_labels[0], error_means[0], yerr=error_bars[0], fmt='o', 
                color='green', ecolor='green', elinewidth=5, capsize=5, markersize=8, label="TD3")
axs[1].errorbar(user_labels[1], error_means[1], yerr=error_bars[1], fmt='o', 
                color='red', ecolor='red', elinewidth=5, capsize=5, markersize=8, label="DDPG")

axs[1].set_ylabel("Mean Reward",fontsize=ylabel_fontsize)
axs[1].set_title("Mean Reward with Error Bars",fontsize=title_fontsize, fontweight='bold')
axs[1].grid(axis='y')

# Rotate x-axis labels by 45 degrees
axs[1].set_xticks(range(len(user_labels)))  # Ensure correct ticks for labels
axs[1].set_xticklabels(user_labels, rotation=45, ha="right",fontsize=xlabel_fontsize)

axs[1].tick_params(axis='x', labelsize=x_and_y_tick_fontsize)
axs[1].tick_params(axis='y', labelsize=x_and_y_tick_fontsize)
